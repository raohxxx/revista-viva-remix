...
import type { RiskLevel, SignalKey, SubscriberWithRisk } from "@/types/domain";
...
export function applyFilters(
  ...
  return items.filter((item) => {
    if (search && !item.subscriber.customer_code.toLowerCase().includes(search)) return false;
    ...
    if (filters.intervention !== "all" && item.interventionStatus !== filters.intervention) return false;
    if (filters.signal !== "all" && item.prediction.principalSignalKey !== filters.signal) return false;
...
export function sortItems(
  ...
  const value = (item: SubscriberWithRisk): number => {
    switch (key) {
      ...
      case "value":
        return item.subscriber.monthly_value;
      default:
        return 0;
    }
  };
